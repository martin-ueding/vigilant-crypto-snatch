...
class Price(Base):
    ...
    def __str__(self):
        return '{}: {} EUR/ETH'.format(self.timestamp, self.last)
...
class Trade(Base):
    ...
    eth = sqlalchemy.Column(sqlalchemy.Float)
    eur = sqlalchemy.Column(sqlalchemy.Float)

	
def open_db_session():
    db_path = os.path.expanduser('~/.local/share/vigilant-crypto-snatch/eth_db.sqlite')
...
def retrieve_historical(then, api_key):
    ...
    url = 'https://min-api.cryptocompare.com/data/histohour?api_key={}&fsym=ETH&tsym=EUR&limit=1&toTs={}'.format(
        api_key,
        timestamp)
...
def try_buy(trading_client, price, trigger, session, now, then):
    ...
    print('Buying {} ETH for {} EUR!'.format(eth, trigger['eur']))

    try:
        buy(trading_client, eth)
...
def buy(trading_client, eth):
    response = trading_client.buy_market_order(eth, base='eth', quote='eur')
...
def check_for_drops(config, session, public_client, trading_client):
    ...
    '''
    ticker = public_client.ticker(base='eth', quote='eur')
