...
def mock_price(then: datetime.datetime):
    ...
    amount = 10000 * math.exp(t / (3600 * 24 * 365)) \
...
class MockHistorical(vigilant_crypto_snatch.historical.HistoricalSource):
    def get_price(self, then: datetime.datetime, coin: str, fiat: str) -> vigilant_crypto_snatch.datamodel.Price:
...
class MockMarketplace(vigilant_crypto_snatch.marketplace.Marketplace):
    def get_spot_price(self, coin: str, fiat: str) -> vigilant_crypto_snatch.datamodel.Price:
